    ...
    <v-container>
        ...
        <div class="panel">
            <div class="gs-bundle-of-buttons" style="max-height:10vh;">
                ...
                <v-btn style="margin-left: 5px;" @click="updateContractDialog = true" class="contrast-primary-text" small color="primary" :disabled="!hasRole('Actor Name')">
                    <v-icon small>mdi-minus-circle-outline</v-icon>계약 수정
                </v-btn>
                <v-dialog v-model="updateContractDialog" width="500">
                    <UpdateContract
                        @closeDialog="updateContractDialog = false"
                ...
                <v-table>
                    ...
                    <tbody>
                        <tr v-for="(val, idx) in value" :key="val" @click="changeSelectedRow(val)" :style="val === selectedRow ? 'background-color: #f0f3ff;':''">
                            ...
                            <td class="whitespace-nowrap" label="종료일">{{ val.endDate }}</td>
...
export default {
    ...
    methods:{
        updateContract(params){
            try{
                this.repository.invoke(this.getSelectedItem(), "updateContract", params)
                this.$EventBus.$emit('show-success','UpdateContract 성공적으로 처리되었습니다.')
                this.updateContractDialog = false
            }catch(e){
                console.log(e)
            }
